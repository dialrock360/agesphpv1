    ...
    class SM_Admin extends Controller{
               ...
               // print_r($_POST);
                ...
                if($param=='config')
                {
                    ...
                    if(isset($_POST['crud']) && $_POST['crud']=='yes')//'valider' est le name du champs submit du formulaire add.html
                    {
                        ...
                        $docrud=true;
                        // echo  $dbtarget ;
                        $tabls=$this->fndb->table_listepure($dbtarget);
                        foreach($tabls as $entitie){
                            $Entity = ucfirst(strtolower($entitie["tablename"]));
                            ...
                            //echo  '<hr>';
                            $this->fnent->create_entitie($Entity,$coltable, null,1,true,true);
                            $this->fnmd->create_model($Entity,$coltable,$dbtarget);
                            $this->fctrt->create_controller($Entity,$coltable,$dbtarget);
                            $this->fvw->create_view($Entity,$dbtarget,'crud',1);
        ...
        function generate_entitie($tablename)
        {
            ...
            if(isset($_POST['charger'])){
                ...
                $otherdatabase=(isset($_POST["otherdatabasename"]) && $_POST["otherdatabasename"]!='')?$_POST["otherdatabasename"]:'';
                $curendb=(isset($_POST["curendatabasename"]) && isset($_POST["use_curendatabase"]))?$_POST["curendatabasename"]:'';
