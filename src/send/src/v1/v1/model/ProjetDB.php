  ...
  use src\entities\Projet;
    ...
    /*==================Classe creer par Samane samane_ui_admin le 05-12-2019 02:38:08=====================*/
        class ProjetDB extends Model {
				...
				/*================== Many to one projet =====================*/
					public function listeProjetByPersonneId($id){
					$sql = "SELECT * FROM projet WHERE  projet.id_chef_projet = ".$id."  ";
					                if($this->db != null)
					                    {
					                        return $this->db->query($sql)->fetchAll();
					                    }else{
				...
				/*================== One to many projet =====================*/
					public function listePersonneByProjetId($id_chef_projet){
					$sql = "SELECT * FROM personne WHERE  personne.id_chef_projet = ".$id_chef_projet."  ";
					                if($this->db != null)
					                    {
					                        return $this->db->query($sql)->fetchAll();
					                    }else{
...
,
               ...
               public function updateprojet($projet){
                        $sql = "UPDATE projet  SET  projet.id_programme =  '".$projet->getId_programme()."' ,projet.nom_projet =  '".$projet->getNom_projet()."' ,projet.cout_projet =  '".$projet->getCout_projet()."' ,projet.valeur_projet =  '".$projet->getValeur_projet()."' ,projet.date_projet =  '".$projet->getDate_projet()."' ,projet.datefin_projet =  '".$projet->getDatefin_projet()."' ,projet.etat_projet =  '".$projet->getEtat_projet()."' ,projet.id_chef_projet =  '".$projet->getId_chef_projet()."' ,projet.flag_projet =  '".$projet->getFlag_projet()."'   WHERE   projet.id =  ".$projet->getId()."  ";
