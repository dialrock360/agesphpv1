...
function Test_fac($date,$nommv,$obj,$mtsch,$cont)
{
    ...
    $resref = "SELECT * FROM mouvement WHERE FLAG=0 AND DATE_DEL='$date' AND NOMMV='$nommv'  AND OBJET='$obj' AND MTSCH='$mtsch' AND CONTEN='$cont'";
...
{
    $idfa=(!isset($id) || empty($id) ) ?0:$id;

//SELECT `IDE`, `IDFA`, `IDMOUV`, `DEPENSE`, `GAINS`, `STOCK`, `DATEE` FROM `etat_compte` WHERE 1
    
  //echo  $resref = "SELECT IDE FROM `etat_compte` WHERE `DATEE` ='$date' AND `IDFA` =$id AND `IDMOUV` =$idmv ".'<hr>';
...
function Test_qntPrd($idp,$qnt,$nommv)
{
//
//SELECT `IDP`, `IDC`, `ID_CAT`, `DESI`, `PHOTO`, `PRIXA`, `PRIXV`, `QNT`, `COMPOSER`, `FTECH`, `FLAG`, `IDFA`, `NOM_CAT`, , `NOMF`, `COLOR`, `NOMC` FROM `v_produit` WHERE 1
    ...
    $resref = "SELECT * FROM `v_produit` WHERE   IDP=".$idp;
    if($nommv=='facture'){
        $countref = getter($resref,$returntype='single');
        ...
        if($countref != null)
        {
            if($ret['ACHAT']==1 && $ret['VENTE']==1 && $ret['COMPT']==1)
            {
               if($QNT <  $qnt)
                {
                    $ret['test']=1;
            ...
            if($ret['ACHAT']==1 && $ret['VENTE']==0 && $ret['COMPT']==0)
            {
                $ret['test']=1;
            }
            if($ret['ACHAT']==0 && $ret['VENTE']==1 && $ret['COMPT']==0)
            {
                $ret['test']=1;
            }
